...
class MiniMaxClient:
    ...
    def analyze_buyer_persona(
        ...
        prompt = build_persona_prompt_v3(user_nick, profile_data, recent_chats, orders or [], is_incremental=is_incremental, max_context_chars=max_chars)
...
"""
    ...
    def analyze_sentiment_intent(
        ...
        - Prompt 与 DeepSeek 完全一致，保证两个模型行为一致。
...
"""
    ...
    def _lookup_intent_value(self, parsed: Dict[str, Any], canonical_key: str) -> Any:
        aliases = {
            ...
            "Inventory Inquiry": [
                "Inventory Inquiry", "Inventory", "Stock Inquiry", "Stock Check",
                "inventory_inquiry", "inventoryInquiry", "库存查询", "库存", "查询库存"
            ],
        }

-- ============================================
-- snapshot_target_buyers_history procedure
-- ============================================
-- 用途：每天 11:30 把当天 target_buyers_precomputed 的真 ASOF 状态
--       写入 target_buyers_precomputed_history 表
-- 关键：参数化 d = CURDATE()，将 procedure 内的 NOW() 替换为 d
-- 幂等：ON DUPLICATE KEY UPDATE 重跑不重复
-- 告警：失败时调用 send_alert_email() 发邮件
-- ============================================

DROP PROCEDURE IF EXISTS snapshot_target_buyers_history;

DELIMITER $$

CREATE PROCEDURE snapshot_target_buyers_history()
BEGIN
    ...
    BEGIN
        ...
        CALL send_alert_email(
            '[smokesignal] snapshot_target_buyers_history 失败',
            CONCAT('snapshot_date=', v_snapshot_date, ', error: ', v_error_msg)
        );
        RESIGNAL;
    END;

    SELECT CONCAT('开始 snapshot, date=', v_snapshot_date) AS message;
    ...
    -- Smoker: 之前买过 Pipes/Lighters (排除 FF/SC 主导客户)
    ...
    -- VIC: rolling 24M 净销售 >= 30K (排除 FF/SC 主导客户)
    ...
    SELECT CONCAT('目标池: ', v_target_count, ' 人') AS message;
    ...
    WITH base AS (
        SELECT
            ...
            cats.second_cat,
            cats.third_cat,
            -- R/F/M scores (主表 procedure 同款阈值)
            CASE
    ...
    -- 输出统计
    SELECT CONCAT('snapshot 完成: target_count=', v_target_count, ', date=', v_snapshot_date) AS message;
    SELECT CONCAT('耗时: ', TIMESTAMPDIFF(SECOND, v_start_time, CURRENT_TIMESTAMP), ' 秒') AS duration;

    -- 验证: 今天必须 > 0 行
    IF (SELECT COUNT(*) FROM target_buyers_precomputed_history WHERE snapshot_date = v_snapshot_date) = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'snapshot inserted 0 rows';
    END IF;
END$$

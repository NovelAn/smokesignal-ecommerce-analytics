...
const ChatAnalysis: React.FC = () => {
  ...
  // 当选中的用户改变时，获取其详细信息（始终获取缓存的AI分析结果）
  const { data: buyerProfile, isLoading: profileLoading, error: profileError, refetch: refetchProfile } = useDataFetchingWithRetry(
    async () => {
      ...
      // 始终传入include_ai=true以获取已缓存的AI分析结果
      const profile = await apiClient.getBuyerProfile(currentSession.user_nick, true);
    ...
    2,
    [currentSession?.user_nick] // 只在用户改变时重新获取（AI开关只控制强制刷新，不影响缓存结果获取）
  );
  ...
  const intentDistributionData = useMemo(() => {
    ...
    // If no real data, use mock data from currentSession
    if (!rawDistribution || typeof rawDistribution !== 'object') {
      return currentSession?.profile?.intent_scores || [];
    }
  ...
  }, [buyerProfile, currentSession?.profile?.intent_scores]);

  // Get dominant intent from API or fallback to currentSession
  const dominantIntent = (buyerProfile as any)?.ai_dominant_intent || currentSession?.dominantIntent || 'Unknown';
  ...
  const r24mAov = (enrichedProfile as any)?.rolling_24m_orders > 0
    ? ((enrichedProfile as any).rolling_24m_netsales || 0) / (enrichedProfile as any).rolling_24m_orders
    : 0;
                         ...
                         >
                            {profileLoading ? (
                              ...
                              <div className="flex flex-col gap-4">
                                ...
                                <div className="bg-yellow-50 p-3 rounded border border-yellow-100 items-start mt-2">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-5 gap-2">
                                        ...
                                        <div className="bg-gradient-to-br from-violet-50 to-violet-100 p-3 rounded border border-violet-200">
                                            ...
                                            <div className="text-base font-mono font-bold text-violet-900">{(((enrichedProfile as any)?.rolling_24m_refund_rate || 0) * 100).toFixed(1)}%</div>
                                        </div>
                                    </div>
                                </div>
                                <div>
